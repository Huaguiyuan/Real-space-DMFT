...
module RDMFT_WRAP_ED
  ...
  interface ed_solve_impurity
     module procedure ed_solve_impurity_sites,ed_solve_sc_impurity_sites
  end interface ed_solve_impurity

  interface ed_fit_bath
     module procedure ed_fit_bath_sites,ed_fit_bath_sc_sites
  end interface ed_fit_bath

  interface ed_get_gloc_lattice
     module procedure ed_get_gloc_lattice,ed_get_gloc_sc_lattice
  end interface ed_get_gloc_lattice

  interface ed_get_gloc_slab
     module procedure ed_get_gloc_slab,ed_get_gloc_sc_slab
  end interface ed_get_gloc_slab

  public :: ed_solve_impurity
  public :: ed_fit_bath
  public :: ed_get_gloc_lattice
  public :: ed_get_gloc_slab
...
contains
  ...
  ! lattice site using ED. Normal and Superconducting phase.
  !+------------------------------------------------------------+!
  subroutine ed_solve_impurity_sites(bath_,Smats,Sreal,nii,dii,eloc,Usite)
    real(8)                  :: bath_(:,:,:),bath_tmp(size(bath_,2),size(bath_,3))
    real(8),optional         :: eloc(size(bath_,1))
    real(8),optional         :: Usite(size(bath_,1))
    complex(8),intent(inout) :: Smats(size(bath_,1),Lmats)
    complex(8),intent(inout) :: Sreal(size(bath_,1),Lreal)
    real(8)                  :: nii(size(bath_,1)),dii(size(bath_,1))
    !MPI 
    complex(8)               :: Smats_tmp(size(bath_,1),Lmats)
    complex(8)               :: Sreal_tmp(size(bath_,1),Lreal)
    real(8)                  :: nii_tmp(size(bath_,1)),dii_tmp(size(bath_,1))
    !
    integer                  :: ilat,i,Nsites
    logical                  :: check_dim
    ...
    call MPI_ALLREDUCE(Smats_tmp,Smats,Nsites*Lmats,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,MPIerr)
    call MPI_ALLREDUCE(Sreal_tmp,Sreal,Nsites*Lreal,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,MPIerr)
    call MPI_ALLREDUCE(nii_tmp,nii,Nsites,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,MPIerr)
    call MPI_ALLREDUCE(dii_tmp,dii,Nsites,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,MPIerr)
  end subroutine ed_solve_impurity_sites

  subroutine ed_solve_sc_impurity_sites(bath_,Smats,Sreal,nii,dii,pii,eloc,Usite)
    real(8)                  :: bath_(:,:,:),bath_tmp(size(bath_,2),size(bath_,3))
    real(8),optional         :: eloc(size(bath_,1))
    real(8),optional         :: Usite(size(bath_,1))
    complex(8),intent(inout) :: Smats(2,size(bath_,1),Lmats)
    complex(8),intent(inout) :: Sreal(2,size(bath_,1),Lreal)
    real(8)                  :: nii(size(bath_,1)),dii(size(bath_,1)),pii(size(bath_,1))
    !MPI
    complex(8)               :: Smats_tmp(2,size(bath_,1),Lmats)
    complex(8)               :: Sreal_tmp(2,size(bath_,1),Lreal)
    real(8)                  :: nii_tmp(size(bath_,1)),dii_tmp(size(bath_,1)),pii_tmp(size(bath_,1))
    !
    integer                  :: ilat,i,Nsites
    logical                  :: check_dim
    character(len=5)         :: tmp_suffix
    ...
    enddo
    if(mpiID==0)call stop_timer
    call MPI_ALLREDUCE(Smats_tmp,Smats,2*Nsites*Lmats,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,MPIerr)
    call MPI_ALLREDUCE(Sreal_tmp,Sreal,2*Nsites*Lreal,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,MPIerr)
    call MPI_ALLREDUCE(nii_tmp,nii,Nsites,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,MPIerr)
    call MPI_ALLREDUCE(dii_tmp,dii,Nsites,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,MPIerr)
    call MPI_ALLREDUCE(pii_tmp,pii,Nsites,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,MPIerr)
  end subroutine ed_solve_sc_impurity_sites
  ...
  !+-----------------------------------------------------------------------------+!
  subroutine ed_fit_bath_sites(bath_,Delta,Gmats,Greal,Smats,Sreal,Eloc)
    real(8)                  :: bath_(:,:,:),bath_tmp(size(bath_,2),size(bath_,3))
    real(8),optional         :: eloc(size(bath_,1))
    complex(8),intent(inout) :: Delta(size(bath_,1),Lmats)
    complex(8),intent(inout) :: Gmats(size(bath_,1),Lmats)
    complex(8),intent(inout) :: Greal(size(bath_,1),Lreal)
    complex(8),intent(inout) :: Smats(size(bath_,1),Lmats)
    complex(8),intent(inout) :: Sreal(size(bath_,1),Lreal)
    complex(8)               :: Delta_tmp(size(bath_,1),Norb,Norb,Lmats)
    integer                  :: ilat,i,Nsites
    logical                  :: check_dim
    character(len=5)         :: tmp_suffix
    Nsites=size(bath_,1)
    ...
    call MPI_ALLREDUCE(Delta_tmp(:,1,1,1:Lmats),Delta,Nsites*Lmats,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,MPIerr)
  end subroutine ed_fit_bath_sites

  subroutine ed_fit_bath_sc_sites(bath_,Delta,Gmats,Greal,Smats,Sreal,Eloc)
    real(8)                  :: bath_(:,:,:),bath_tmp(size(bath_,2),size(bath_,3))
    real(8),optional         :: eloc(size(bath_,1))
    complex(8),intent(inout) :: Delta(2,size(bath_,1),Lmats)
    complex(8),intent(inout) :: Gmats(2,size(bath_,1),Lmats)
    complex(8),intent(inout) :: Greal(2,size(bath_,1),Lreal)
    complex(8),intent(inout) :: Smats(2,size(bath_,1),Lmats)
    complex(8),intent(inout) :: Sreal(2,size(bath_,1),Lreal)
    complex(8)               :: Delta_tmp(2,size(bath_,1),Norb,Norb,Lmats)
    ...
    call MPI_ALLREDUCE(Delta_tmp(:,:,1,1,:),Delta,2*Nsites*Lmats,MPI_DOUBLE_COMPLEX,MPI_SUM,MPI_COMM_WORLD,MPIerr)
  end subroutine ed_fit_bath_sc_sites
  ...
  !+-----------------------------------------------------------------------------+!
  ! PURPOSE: Evaluate the local GF for all cases: lattice or slab, attractive or 
  ! repulsive, building and inverting a large matrix.
  !+-----------------------------------------------------------------------------+!
  subroutine ed_get_gloc_lattice(Gmats,Greal,Smats,Sreal,Eloc)
    ...
    Gmats=zero
    Greal=zero
    call get_gloc_mats(Eloc_,Smats,Gmats)
    call get_gloc_real(Eloc_,Sreal,Greal)
  end subroutine ed_get_gloc_lattice

  subroutine ed_get_gloc_slab(epsik,wt,Gmats,Greal,Smats,Sreal,Eloc)
    ...
    if(mpiID==0)call start_timer
    do ik=1,Lk
       call get_gloc_mats(eloc_,Smats,Gmats,epsik(ik),wt(ik))
       call get_gloc_real(eloc_,Sreal,Greal,epsik(ik),wt(ik))
       if(mpiID==0)call eta(ik,Lk,unit=LOGfile)
    end do
    if(mpiID==0)call stop_timer
  end subroutine ed_get_gloc_slab

  subroutine ed_get_gloc_sc_lattice(Gmats,Greal,Smats,Sreal,Eloc)
    real(8),optional         :: Eloc(Nlat)
    real(8)                  :: Eloc_(Nlat)
    ...
    Gmats=zero
    Greal=zero
    call get_sc_gloc_mats(eloc_,Smats,Gmats)
    call get_sc_gloc_real(eloc_,Sreal,Greal)
  end subroutine ed_get_gloc_sc_lattice

  subroutine ed_get_gloc_sc_slab(epsik,wt,Gmats,Greal,Smats,Sreal,Eloc)
    real(8),dimension(:)           :: epsik
    real(8),dimension(size(epsik)) :: wt
    real(8),optional               :: Eloc(Nlat)
    real(8)                        :: Eloc_(Nlat)
    ...
    if(mpiID==0)write(LOGfile,*)"Get local GF (id=0):"
    do ik=1,Lk
       call get_sc_gloc_mats(eloc_,Smats,Gmats,epsik(ik),wt(ik))
       call get_sc_gloc_real(eloc_,Sreal,Greal,epsik(ik),wt(ik))
       if(mpiID==0)call eta(ik,Lk,unit=LOGfile)
    end do
    if(mpiID==0)call stop_timer
  end subroutine ed_get_gloc_sc_slab
