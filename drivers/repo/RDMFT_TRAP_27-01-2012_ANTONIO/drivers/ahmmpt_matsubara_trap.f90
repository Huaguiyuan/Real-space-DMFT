...
contains
  ...
  subroutine print_sc_out(converged)
    ...
    real(8)                  :: delta_center,delta_tot,delta_max,delta_av
    ...
    if(mpiID==0)then
       ...
       n_tot=0.d0; delta_tot=0.d0; occupied=0 
       ...
       print*,"occupancy =",occupied, " over ",Ns
       if ((occupied/Ns).gt.0.9d0) print*,"WARNING: MAYBE TOUCHING THE TRAP BOUNDARIES"
       print*,"Average density =",n_av
       print*,"N_totale =",n_tot
       print*,"Delta_av =",delta_av

       call splot(trim(adjustl(trim(name_dir)))//"/navVSiloop.ipt",iloop,n_av,append=TT)
