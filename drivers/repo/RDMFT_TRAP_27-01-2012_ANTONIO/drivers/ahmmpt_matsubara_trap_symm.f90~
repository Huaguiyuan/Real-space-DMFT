...
program adhmipt_matsubara_trap
  ...
  !===============================================================
  include "init_global_trap_symm.f90" 
...
!      ho calcolato le self-energie per i siti non equivalenti
       ...
       do i=1,L
          call c_simmetrize(sigma(1,:,i))
          call c_simmetrize(sigma(2,:,i))
       enddo
   
!      implementa le simmetrie del cubo sulle self-energie 
...
!       endif 
  ...
  subroutine print_sc_out(converged)
    ...
    if(mpiID==0)then
       ...
       call system("rm -fv *.ipt *.ipt.gz")
        
!-------------------------------------------------------------------------------------------       
!      to be removed.. after debugging store only compact form of
...
!******************************************************************************
       ...
       !   data processing and debugging
          ...
          call splot("Sigma_iw_center.ipt",wm,sigma(1,center,1:L),append=FF)
          call splot("Self_iw_center.ipt",wm,sigma(2,center,1:L),append=FF)

       
          call splot("Gloc_iw_border.ipt",wm,fg(1,border,1:L),append=FF)
          call splot("Floc_iw_border.ipt",wm,fg(2,border,1:L),append=FF)
          call splot("Sigma_iw_border.ipt",wm,sigma(1,border,1:L),append=FF)
          call splot("Self_iw_border.ipt",wm,sigma(2,border,1:L),append=FF)

       
          call splot("Gloc_iw_corner.ipt",wm,fg(1,corner,1:L),append=FF)
          call splot("Floc_iw_corner.ipt",wm,fg(2,corner,1:L),append=FF)
          call splot("Sigma_iw_corner.ipt",wm,sigma(1,corner,1:L),append=FF)
          call splot("Self_iw_corner.ipt",wm,sigma(2,corner,1:L),append=FF)
...
!          call splot(trim(adjustl(trim(name_dir)))//"/LF.ipt",fg(2,1:Ns,1:L))
          ...
          call system("mv -vf *.ipt "//trim(adjustl(trim(name_dir)))//"/ ")
