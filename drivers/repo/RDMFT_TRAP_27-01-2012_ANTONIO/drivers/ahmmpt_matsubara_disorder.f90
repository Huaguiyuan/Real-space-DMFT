...
contains
  ...
  subroutine print_sc_out(converged)
    ...
    if(mpiID==0)then
       ...
       do i=1,Ns
          call splot("Gloc_iw_site.ipt",wm,fg(1,i,1:L),append=FF)
          call splot("Floc_iw_site.ipt",wm,fg(2,i,1:L),append=FF)
          call splot("Sigma_iw_site.ipt",wm,sigma(1,i,1:L),append=FF)
          call splot("Self_iw_site.ipt",wm,sigma(2,i,1:L),append=FF)
       enddo
