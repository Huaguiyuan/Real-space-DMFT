...
program ahm_matsubara_disorder
  ...
  allocate(nii_tmp(Ns),cdii_tmp(Ns))
  ...
  do while(.not.converged)
     ...
     converged = check_convergence_scalar(abs(cdii),eps_error,Nsuccess,nloop,id=0)
...
contains
  ...
  subroutine solve_sc_impurity_mpi()
    ...
    if(disorder)then
       ...
       !
       call MPI_REDUCE(cdii_tmp,cdii,Ns,MPI_DOUBLE_COMPLEX,MPI_SUM,0,MPI_COMM_WORLD,MPIerr)
       call MPI_BCAST(cdii,Ns,MPI_DOUBLE_COMPLEX,0,MPI_COMM_WORLD,mpiERR)
    else
  ...
  subroutine solve_per_site(is)
    ...
    real(8),dimension(0:L)                       :: fgt,fg0t !G
    complex(8),dimension(0:L)                    :: fft,ff0t !F
    real(8)                                      :: n,n0
    ...
    !sold(:,is,:) = sigma(:,is,:)
    !
    call fftgf_iw2tau(fg(1,is,:),fgt(0:),beta)
    call fftff_iw2tau(fg(2,is,:),fft(0:),beta)
    n = -fgt(L) ; delta = -u*fft(L)
    !
    nii_tmp(is)=2.d0*n; cdii_tmp(is)=delta
    !
    fg0=zero ; calG=zero
    det       = abs(fg(1,is,:))**2    + abs(fg(2,is,:))**2
    ...
    sc0_phase=phase(delta0)

    write(750,"(2I4,6(f16.12))",advance="yes")mpiID,is,n,n0,sc_mod,sc_phase,sc0_mod,sc0_phase
    !
  ...
  subroutine print_sc_out(converged)
    ...
    if(mpiID==0)then
       ...
       call splot(trim(adjustl(trim(name_dir)))//"/deltaVSiloop.data",iloop,sc_mod,append=TT)
       call splot(trim(adjustl(trim(name_dir)))//"/phaseVSiloop.data",iloop,sc_phase,append=TT)
       call splot(trim(adjustl(trim(name_dir)))//"/nVSisite.data",nii)
       call splot(trim(adjustl(trim(name_dir)))//"/deltaVSisite.data",abs(cdii))
       call splot(trim(adjustl(trim(name_dir)))//"/phaseVSisite.data",phase(cdii))
  ...
  elemental function phase(cnum) result(rphase)
    complex(8),intent(in) :: cnum
    real(8)               :: rphase
    rphase=atan2(dimag(cnum),real(cnum,8))
  end function phase
